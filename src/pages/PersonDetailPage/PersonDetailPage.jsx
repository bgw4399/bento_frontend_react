...
export default function PersonDetailPage() {
    ...
    const fetchDataById = useCallback(async (id) => {
        // [주의] '/api/visit/:id' 엔드포인트는 현재 백엔드(person.controller.ts)에 존재하지 않습니다.
        // 이 기능을 사용하려면 백엔드에 새 API 구현이 필요합니다.
        setMessage("Loading Table...");
        setIsLoading(true);
        try {
            const res = await fetch(`${API_URI}/api/visit/${id}/`); // 이 API는 구현이 필요합니다.
